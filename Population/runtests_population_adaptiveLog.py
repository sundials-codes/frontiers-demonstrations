...
#------------------------------------------------------------------------------------------------------------------------------------
# This script generates the log files needed to plot the all the steps sizes used in the adaptive run.
#-------------------------------------------------------------------------------------------------------------------------------------
...
for k_name, k_val in diff_coef.items():
    for runV_name, runV_val in adaptive_params.items():
            for solver_adapt in solvertype:
              stat = runtest(solver_adapt, runV_name, runV_val, k_name, k_val, common, showcommand=True, sspcommand=True)
